...
class SirlTverskySim(nn.Module):
    def __init__(self, 
                 input_dim=19,
                 hidden_dim=128,
                 latent_dim=6,
        ...
        self.tversky_sim = tnn.TverskySimilarity(
            embedding_dim=latent_dim, # TverskySim is computed on phi_a, phi_p, phi_n instead of raw traj
    ...
    def similarity(self, a, b):
        """
        a and b are raw trajectories of `input_dim`
        """
        phi_a = self.encoder(a)
        phi_b = self.encoder(b)
        alpha = F.softplus(self.raw_alpha)
        beta  = F.softplus(self.raw_beta)
        return self.tversky_sim(phi_a, phi_b, alpha=alpha, beta=beta)
    ...
    def distance(self, a, b):
        ...
        a and b are raw trajectories; similarity() embeds them with the trained encoder
        """
...
@config_overridable
def random_init_no_training_sirl_tversky_sim(
    config,
    input_dim=19,
    hidden_dim=128,
    latent_dim=6,
...
def load_sirl_tversky_sim(path, device=None):
    ckpt = torch.load(path, map_location=device, weights_only=False)
    model = SirlTverskySim(**ckpt['hparams'])
